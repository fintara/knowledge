...
class Knowledge (vararg val knowledge: List<Int>) {
    ...
    fun discoverProperties(support: Double): Set<Pattern> {
        tailrec fun go(patterns: Set<Pattern>, solutions: List<Set<Pattern>> = emptyList()): Set<Pattern> {
            ...
            val next = checked.simplify().filter { supportFor(it) >= support }.toSet()
    ...
    fun discoverRules(support: Double, confidence: Double): Set<Rule> {
        ...
        val rules = (0 until size).flatMap { pivot ->
            properties
                .map { Rule(
                    it.subList(0, pivot)
                        .mapIndexed { index, value -> if (value is Value.Fixed) Attribute(index, value.relation, value.value) else null }
                        .filterNotNull()
                        .toSet(),

                    it.subList(pivot, size)
                        .mapIndexed { index, value -> if (value is Value.Fixed) Attribute(index + pivot, value.relation, value.value) else null }
                        .filterNotNull()
                        .toSet()
                ) }
        }
    ...
    fun getSimplePatterns(): Set<Pattern> {
        ...
        return (0 until size).flatMap { attr ->
            Relation.values().flatMap { relation -> knowledge.getValues(attr).map { empty.fix(attr, relation, it) } }
        }.toSet()
    }
    ...
    private fun checkRowPattern(row: List<Int>, pattern: Pattern): Int {
        val sum = row.zip(pattern).map { (a, b) ->
            when (b) {
                is Value.Any -> 0
                is Value.Fixed -> if (b.relation.compare(a, b.value)) 1 else 0
            }
        }.sum()
